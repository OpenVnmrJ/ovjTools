/*
 * @(#)osapi_ntptime_impl.h    generated by: makeheader    Mon Dec  3 23:07:47 2007
 *
...
{ \
    register RTI_UINT32 RTINtpTime_temp = nsec; \
    (time).sec  = s; \
    (time).frac = (RTINtpTime_temp<<2)+ \
		  ((RTINtpTime_temp  + \
			(RTINtpTime_temp>>3) + \
		  (RTINtpTime_temp>>5)+ \
		  (RTINtpTime_temp>>6)+ \
		  ...
		  (RTINtpTime_temp>>21)+ \
		  (RTINtpTime_temp>>27)+ \
		  (RTINtpTime_temp>>30))>>2); \
}

#define RTINtpTime_unpackToNanosec(s, nsec, time) \
{ \
    register RTI_UINT32 RTINtpTime_temp = (time).frac>>1; \
    register RTI_UINT32 RTINtpTime_delta; \
    s     = (time).sec; \
    RTINtpTime_delta = \
	    (RTINtpTime_temp) + \
	    (RTINtpTime_temp>>4) + \
